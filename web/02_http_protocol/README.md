...
> [WSGI 最小应用](../01_wsgi_barebones/README.md)里，`environ` 和 body 之间的
...
Web 框架开发者每天和 HTTP 打交道，但多数人对报文的认知停在"headers 是个 dict"。一句话心智模型：**HTTP 报文 = 起始行 + 头 + 空行 + 体，前段按 CRLF 逐行读，"体读多长"由头里的定界机制说了算**。三个必须下到字节层才看得见的事实：报文是四段式文本（所谓解析就是按 CRLF 切字符串）；body 的结束由两种机制定界（`Content-Length` 先知长度，或 `Transfer-Encoding: chunked` 分块流式）；连接可以复用（keep-alive），复用与否服务端 accept 计数器一测便知。

## Why

"一个请求一条 TCP 连接"是 1.0 时代的老黄历；头里的 `Connection` 头决定连接复用，`Content-Length` 与 `chunked` 是体定界的两种互斥答案。本实验把三件事全部做成可断言的实测。
...
### body 定界：Content-Length vs chunked
...
**Q2: WSGI 里的 `wsgi.input` 和本实验什么关系？**
server（如 wsgiref/gunicorn）做完了本实验服务端的活：按定界机制把 body 从 TCP 流里切出来，包成 `wsgi.input` 文件对象交给应用——所以 Flask 的 `request.data` 直接可读，不用关心 chunked。server/框架替我们管理的状态（Cookie/Session）见 [03 · Cookie 与 Session](../03_cookie_session/README.md)。
