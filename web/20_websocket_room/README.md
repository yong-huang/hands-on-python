...
> HTTP 的"请求-响应"配对在实时场景失灵——聊天室需要**服务端主动推**。
...
轮询/长轮询模拟"实时"的时代已经过去，但 WebSocket 的三个事实不亲手做过就不踏实：**连接是有状态的**——服务端必须维护"房间 = 连接列表"，断开要清理否则人数虚高；**广播就是循环 send**——没有魔法，慢消费者需要单独的背压策略；**断线靠 `WebSocketDisconnect` 异常感知**——它是清理与人数准确性的信号源。一句话心智模型：**握手一次，连接归会话——此后服务端可随时主动推，客户端也可随时发，没有配对关系**。
...
- **未实现 Litestar 对照路径**：清单标注二选一，本站选了 WebSocket 主线；Litestar 的 DI 对照可按 lab 09 的方法自行扩展
...
**Q4: FastAPI 里 WebSocket 端点和 HTTP 端点能共存吗？认证怎么做？**
