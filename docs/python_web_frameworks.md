...
**完成日期**：2026-09-11
**踩坑记录**：三个 standalone app 的坑逐个踩平：Django :memory: 每线程各一份空库（表'消失'）→ 文件库；FastAPI 的 title:str 不拦空串需 handler 补 400；声明式 LOGGING 静音预期 401/400（命令式 setLevel 会被二次 setup 覆盖）。压测 Flask/FastAPI 131 vs Django 121 req/s。
...
**完成日期**：2026-09-11
**踩坑记录**：一次通过（Docker daemon 已就绪）。100 请求分发 4 个 PID；APP_MODE 环境变量 prod/dev 切换断言；nginx 反代按清单标注选做，README 给出配置思路未纳入自动断言。
...
**完成日期**：2026-09-11
**踩坑记录**：三个真实坑：Dockerfile 首行误写 Python docstring；SQLite DateTime 无时区 → aware/naive TypeError，全库统一 naive UTC；容器内 sqlite 父目录不存在启动即崩（mkdir -p 解决）。pytest 19 条全绿。

---

### [x] 项目 20：⚠️ 选做——WebSocket 实时聊天室（或 Litestar 对比）
...
**完成日期**：2026-09-11
**踩坑记录**：选了 WebSocket 主线（Litestar 对照留作扩展）。send_json 的载荷被 receive_text 原样转发 → 改 send_text；广播含发送者回声，断言先消费自己的；sequence 图源 y 最小 160。
...
- [x] **完成项目 1-3** → **协议地基通**：手写过 WSGI 应用、HTTP 报文与会话机制，之后任何框架的"魔法"都不再黑盒
- [x] **完成项目 4-7** → **Flask 交付者**：独立交付带认证、多蓝图、ORM 支撑的 Flask 应用
- [x] **完成项目 8-12** → **FastAPI 工程师**：类型驱动校验、依赖注入、异步执行模型、JWT 认证全部落地，说清请求在 ASGI 栈里的真实位置
- [x] **完成项目 13-16** → **Django 工程师**：MTV 全家桶 + DRF REST API + 信号缓存测试体系
- [x] **完成项目 17-18** → **选型与上线**：三框架同题拿实测数据做选型，生产部署（gunicorn/uvicorn + Docker + nginx）走得通
- [x] **完成项目 19** → **Web 全栈工程师**：独立交付完整、被测、容器化的服务
- [x] **完成项目 20（选做）** → **实时通信入门**：WebSocket 广播或跨框架视野加餐
