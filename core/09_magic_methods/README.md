...
Python 的运算符和内置函数背后都是魔术方法：`a + b` 调用 `a.__add__(b)`，`len(obj)` 调用 `obj.__len__()`，`str(obj)` 调用 `obj.__str__()`。不实现它们，自定义类就只是个"属性袋子"——不能比较、不能做 dict key、调试输出全是内存地址。理解魔术方法可以自定义类的行为，配合 `@total_ordering` 和 `@dataclass` 还能大幅减少样板代码。这也是"== vs is""自定义对象怎么做 dict key"等经典问题的答案源头。
...
> 🌐 **交互版**：[在线打开（GitHub Pages）](https://yong-huang.github.io/hands-on-python/core/09_magic_methods/images/magic_methods.html)
