...
Python 是动态类型语言，但 PEP 484 引入了类型注解（Type Hints），让代码在**不失去动态灵活性**的前提下获得静态检查的能力。类型注解本身不改变运行时行为，它是给 mypy / pyright 等静态检查器和 IDE 用的"提示"。不掌握它会怎样：容器里塞进错误类型只能等线上炸，IDE 补全和重构形同虚设，"讲讲 TypeVar / Protocol"更是 Python 工程师的基本功。typing 模块提供的核心工具：
...
> 🌐 **交互版**：[在线打开（GitHub Pages）](https://yong-huang.github.io/hands-on-python/core/18_typing_generic/images/typing_generic.html)
