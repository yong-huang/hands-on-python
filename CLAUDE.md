...
A Python hands-on tutorial series ("hands-on-python") focused on interview-grade Python language internals: 20 self-contained labs under `core/`. Lab conventions follow the `hands-on-series` skill (`~/.agents/skills/hands-on-series/SKILL.md`) — read that skill before creating new labs or restructuring existing ones.
...
- `core/NN_topic/` — 20 numbered labs (01_decorator_factory … 20_itertools_func), one Python interview topic each. The directory is named `core/` (not the skill's `labs/`) — a deliberate deviation to match this repo's theme.
